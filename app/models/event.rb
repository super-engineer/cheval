class Event < ActiveRecord::Base
  ...
  attr_accessible :description, :done, :name, :scheduled_on, :poster, :event_pictures_attributes, :thumbnail_poster
  ...
  scope :upcoming, where("scheduled_on >= ?", (Date.today-1.days)).order("scheduled_on DESC")
  scope :past, where("scheduled_on < ?", (Date.today-1.days)).order("scheduled_on ASC")
